...
interface ScatterPlotProps {
  ...
  height?: number;
  showLabels?: boolean;
  // For reassessment comparison: show arrows from previous to current position
...
const QUADRANT_BG_COLORS = {
  intentional: "rgba(0, 126, 250, 0.05)",
  command_control: "rgba(245, 166, 35, 0.05)",
  overly_supportive: "rgba(245, 166, 35, 0.05)",
  absent: "rgba(234, 12, 103, 0.05)",
};
...
export default function ScatterPlot({
  ...
  function toPixelY(score: number) {
    // Y is inverted (higher score = higher on screen = lower Y pixel)
    ...
    <div className="relative inline-block">
      <svg width={width} height={height} className="overflow-visible">
        ...
        <rect
          ...
          width={plotWidth / 2}
          height={plotHeight / 2}
          fill={QUADRANT_BG_COLORS.intentional}
        />
        <rect
          x={padding}
          y={padding}
          width={plotWidth / 2}
          height={plotHeight / 2}
          fill={QUADRANT_BG_COLORS.command_control}
        />
        <rect
          x={padding}
          y={midY}
          width={plotWidth / 2}
          height={plotHeight / 2}
          fill={QUADRANT_BG_COLORS.absent}
        />
        <rect
          x={midX}
          y={midY}
          width={plotWidth / 2}
          height={plotHeight / 2}
          fill={QUADRANT_BG_COLORS.overly_supportive}
        />
        ...
        {previousPositions &&
          managers.map((m) => {
            ...
            return (
              <line
                key={`arrow-${m.id}`}
                x1={toPixelX(prev.xScore)}
                y1={toPixelY(prev.yScore)}
                x2={toPixelX(m.xScore)}
                y2={toPixelY(m.yScore)}
                stroke="#101d51"
                strokeWidth="1.5"
                strokeOpacity="0.3"
        ...
        {previousPositions &&
          managers.map((m) => {
            ...
            return (
              <circle
                key={`prev-${m.id}`}
                cx={toPixelX(prev.xScore)}
                cy={toPixelY(prev.yScore)}
                r={6}
                fill={
                  QUADRANT_COLORS[
                    m.quadrant as keyof typeof QUADRANT_COLORS
                  ] || "#101d51"
                }
                opacity={0.2}
                stroke="white"
        ...
        {managers.map((m) => (
          <g key={m.id}>
            <circle
              cx={toPixelX(m.xScore)}
              cy={toPixelY(m.yScore)}
            ...
            {hoveredManager === m.id && (
              <g>
                <rect
                  x={toPixelX(m.xScore) - 60}
                  y={toPixelY(m.yScore) - 38}
                  width="120"
                  height="28"
                  rx="4"
                  fill="#101d51"
